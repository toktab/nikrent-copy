...
describe('layoutDimensions', () => {
  ...
  it('steps the longer of two overlapping lengths out onto its own dimension line', () => {
    const placed = byId(
      layoutDimensions([span('long', [0, 10], [120, 10]), span('short', [10, 5], [110, 5])], view),
    );
    const short = placed.get('short')!;
    const long = placed.get('long')!;
    expect(short.tier).toBe(0);
    expect(long.tier).toBeGreaterThanOrEqual(1);
    expect(long.ext![0][0].x).toBeCloseTo(0);
    expect(long.ext![1][0].x).toBeCloseTo(240);
